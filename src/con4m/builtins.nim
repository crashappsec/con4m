...
type BuiltinDocInfo* = object
  sig:  string
  doc:  string
  tags: seq[string]

builtinDocs: Table[string, BuiltinDocInfo]
...
proc newBuiltIn*(s:     ConfigState,
  ...
  try:
    ...
    builtinDocs[sig] = BuiltinDocInfo(sig: sig, doc: doc, tags: tags)
  except:
...
const defaultBuiltins* = [
  ...
  ("Duration(string) -> Duration",
   BuiltInFn(c4mSToDur),
   "Turns a string into a duration object. The ",
   @["type conversion"]),
