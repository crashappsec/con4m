...
type
  ZOp* {.size: 1.} = enum
     ...
     # This calls a function by heap index / offset. If there's a
     # return value, it's left on the stack.
     Z0Call         = 0x34,
...
proc rawReprInstructions(module: ModuleInfo, ctx: ZObject, fn = 0): Rope =
  ...
  for i, item in module.instructions:
    var address = text(hex(i * sizeof(ZInstruction)))

    if item.op == ZNop and item.arg != 0:
      var str = mem.findStringAt(item.immediate)

      if str.len > 20:
        str = str[0 .. 9] & " … " & str[^9 .. ^1]

      row = @[address, text(" "), text(" "),
                  text(" "), text(" "), strong(str)]
    else:
      var
        arg1 = text(" ")
        arg2 = text(" ")
      ...
      of ZPushO, ZPushF, ZPushVal, ZPushPtr, ZPushSType, ZPushAddr, ZStore,
           ZStoreImm:
        arg1 = text("+" & hex(item.arg))
        ...
        if item.arena == -1:
          ...
          line = em(ctx.funcInfo[fn].syms.lookup(item.arg).getOrElse("(stack)"))
        elif item.arena != 0:
          # line = em(module.syms[item.arg])
          line = em(module.syms.lookup(item.arg).getOrElse("(module)"))
        else:
          #line = em(ctx.globals[item.arg])
          line = em(ctx.globals.lookup(item.arg).getOrElse("(global)"))
      of ZPushStaticPtr:
        arg1 = text("+" & hex(item.arg))
        arg2 = text(mem.findStringAt(item.arg))
      of ZPushImm:
        arg2 = text($item.immediate)
      of ZModuleRet, ZRet, ZNop:
        ty = text(" ")
      of ZJz, ZJnz, ZJ:
        if item.arg > 0:
          arg1 = text("+" & hex(item.arg))
      ...
      row = @[address, em($(item.op)), arg1, arg2, ty,
               line]
...
proc getSymbolArena(ctx: var CodeGenState, sym: SymbolInfo): int =
  ...
  if sym.arena == 0 and not sym.global:
    if ctx.fcur != nil:
      ctx.fcur.syms[sym.offset] = sym.name
    else:
      ctx.mcur.syms[sym.offset] = sym.name
...
proc genPop(ctx: var CodeGenState, sym: SymbolInfo = nil,
  ...
  if sym == nil:
    ...
    ctx.addInstruction(ZStore, sym.getOffset(), tid = ntid, arena = arena)
    ctx.addInstruction(ZPop, tid = ntid)
...
proc genStore(ctx: var CodeGenState, sym: SymbolInfo, tid = sym.tid) =
  ctx.addInstruction(ZStore, sym.getOffset(), tid = tid,
                     arena = ctx.getSymbolArena(sym))
...
proc genFunction(ctx: var CodeGenState, fn: FuncInfo) =
  # TODO:
  #
  # 1) Change the frame / stack pointers.
  # 2) If params are generic, do a type binding.
  # 3) Generate decent labels w/ type info
  # 4) Handle moving the stack / frame pointers back.
  # 5) Leave any return value on the stack.  (lay out return better)
  # 6) Add func addresses to a lookup table.
  discard
