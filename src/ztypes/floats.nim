...
proc cast_f_to_i(pre: pointer): pointer {.cdecl, exportc.} =
  let
    f = cast[float64](pre)
    i = int64(f)
...
proc cast_f_to_u(pre: pointer): pointer {.cdecl, exportc.} =
  let
    f = cast[float64](pre)
    i = uint64(f)
...
proc get_cast_func_float(dt, ot: DataType, err: var string): pointer {.cdecl.} =
  if dt.fTy:
